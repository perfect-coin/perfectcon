            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
						...
						// change PerfectCoin to get rid of automatic generation of change address and instead send change back
...
{
    //presstab PerfectCoin: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    // presstab PerfectCoin - Initialize as static and don't update the set on every run of CreateCoinStake() in order to lighten resource use
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
    {
		...
		//PerfectCoin now iterates each utxo inside of CheckStakeKernelHash()
		if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNewTime, nHashDrift, false, hashProofOfStake))
		{
			...
			//presstab PerfectCoin - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
			uint64 nCoinAge;
			CTxDB txdb("r");
			const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBest, false);
			...
			//presstab PerfectCoin - if MultiSend is set to send in coinstake we will add our outputs here (values asigned further down)
	...
	if(fCombineDust) //presstab PerfectCoin - this combination code iterates through all of your outputs on successful coinstake, so its useful to have user be able to choose whether this is necessary
	{
		BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
		{
